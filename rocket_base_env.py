...
class RocketBaseEnv(gymnasium.Env):
    ...
    def begin_reset(
        ...
        # self.env.register_wind_field_function(simple_wind)
    ...
    def end_reset(
        ...
        for _ in range(140 * self.env_step_ratio):
            self.env.step()
    ...
    def step(self, action: np.ndarray):
        ...
        # if test landing only with some of action elements
        action = np.array([0, 0, 0, action[3], action[4], 0, 0])
        ...
        self.step_count += 1
        if self.termination or self.truncation:
            self.reward += 10.0 * (1 - np.linalg.norm(self.previous_lin_vel))
            # self.reward += 10.0 * (1 - np.linalg.norm(self.previous_ang_vel))
            # self.reward += 10.0 * (1 - np.linalg.norm(self.ang_pos[:2]))
            # self.reward += 10.0 * (np.linalg.norm(self.distance))

        return self.state, self.reward / self.env_step_ratio, self.termination, self.truncation, self.info
